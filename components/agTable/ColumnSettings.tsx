...
const ColumnSettings = ({ columnDefs, gridRef, visibleColumns }: ColumnSettingsProps) => {
  ...
  const [tempColumnsOrder, setTempColumnsOrder] = useState(columnDefs.map((col) => col.field));
  ...
  const handleApply = () => {
    applyChanges(tempColumnsOrder.filter((field): field is string => !!field), tempVisibleColumns);
    setIsOpen(false);
  };
  ...
  return (
<>
       <button onClick={() => setIsOpen(!isOpen)}  title="Column Settings" className={`cursor-pointer rounded-sm hover:opacity-80 ${isOpen ? 'bg-[#6D6E73]/20' : ''}`}  >
        <Settings size="32" color="#35353A" className="transform scale-[.6]"  />
      </button>
      {isOpen && (
        <div className="relative">
        <div ref={menuRef}  className="absolute right-0 mt-4 bg-white shadow-lg px-3 rounded-md border border-gray-300 w-56 z-10">
           <h3 className="h-10 items-center flex border-b border-gray-300 font-bold px-3 mb-2">Configure Table</h3>
          <DndContext collisionDetection={closestCenter} onDragEnd={handleDragEnd}>
            <SortableContext items={tempColumnsOrder.filter((field): field is string => !!field)} strategy={verticalListSortingStrategy}>
              {tempColumnsOrder.filter((field): field is string => !!field).map((field) => {
          ...
          <div className="flex justify-between mt-2 py-3 border-t border-gray-300 ">
            ...
            <button className="bg-[#15274E] text-white px-3 py-2 rounded hover:bg-[#15274E]/80 cursor-pointer" onClick={handleApply}>
              Apply
            </button>
          </div>  
        </div>
